...
// Express route handler safely wrapped
...
app.post("/api/doctorismo/diagnose", async (req: any, res: any) => {
  try {
...
app.post("/api/doctorismo/generate-avatar", async (req: any, res: any) => {
  try {
    const { patientData } = req.body;
    if (!patientData) {
      return res.status(400).json({ error: "Faltan datos del paciente." });
